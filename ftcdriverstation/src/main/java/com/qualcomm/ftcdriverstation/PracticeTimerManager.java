...
public class PracticeTimerManager {
    ...
    public PracticeTimerManager(Context context, ImageView imageView, TextView textView) {
        ...
        this.SOUND_ID_PICK_UP_CTRLS = soundPool2.load(context, R.raw.lady_pick_up_ctrls, 1);
        this.SOUND_ID_TELE_COUNTDOWN = this.soundPool.load(context, R.raw.lady_3_2_1, 1);
        this.SOUND_ID_FIREBELL = this.soundPool.load(context, R.raw.firebell, 1);
        this.SOUND_ID_FACTWHISTLE = this.soundPool.load(context, R.raw.factwhistle, 1);
        this.SOUND_ID_END_MATCH = this.soundPool.load(context, R.raw.endmatch, 1);
        this.SOUND_ID_ABORT_MATCH = this.soundPool.load(context, R.raw.fogblast, 1);
        this.SOUND_ID_START_AUTO = this.soundPool.load(context, R.raw.charge, 1);
        this.SOUND_ID_END_AUTO = this.soundPool.load(context, R.raw.endauto, 1);
        this.SOUND_ID_MC_BEGIN_AUTO = this.soundPool.load(context, R.raw.mc_begin_match, 1);
    }
    ...
    public void autoTimer() {
        synchronized (this.syncobj) {
            if (this.running) {
                ...
                this.timerView.setTextColor(this.theContext.getResources().getColor(R.color.practice_timer_font_white));
                this.countDownTimer = new CountDownTimer(FtcDriverStationActivityBase.OpModeCountDownTimer.MS_COUNTDOWN_INTERVAL, 100) {
                    public void onTick(long j) {
        ...
        synchronized (this.syncobj) {
            if (this.running) {
                ...
                this.countDownTimer = new CountDownTimer(90000, 100) {
                    public void onTick(long j) {
                        PracticeTimerManager.this.timerView.setText(PracticeTimerManager.this.formatTimeLeft(j + 1000 + FtcDriverStationActivityBase.OpModeCountDownTimer.MS_COUNTDOWN_INTERVAL));
                    }
